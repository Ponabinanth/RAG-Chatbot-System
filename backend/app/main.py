...
from app.schemas import ChatRequest, ChatResponse, DocumentSummary, HealthResponse, IngestRequest, MetricsResponse, PredictionResponse, UploadResponse
...
async def get_metrics() -> MetricsResponse:
    ...
    return MetricsResponse(
        ...
        total_tokens_used=int(metrics["total_tokens_used"]),
        total_estimated_cost_usd=round(metrics["total_estimated_cost_usd"], 4),
    )

@app.get("/metrics/predict", response_model=PredictionResponse)
async def predict_metrics() -> PredictionResponse:
    chats = int(metrics["chats_served"])
    tokens = int(metrics["total_tokens_used"])
    cost = float(metrics["total_estimated_cost_usd"])
    
    if chats == 0:
        return PredictionResponse(prediction="No chat data available to predict future usage. Start chatting!")
        
    avg_tokens = tokens / chats
    predicted_weekly_chats = 1000
    predicted_weekly_cost = (cost / chats) * predicted_weekly_chats
    
    if settings.openai_api_key:
        try:
            from openai import OpenAI
            client = OpenAI(api_key=settings.openai_api_key)
            prompt = f"System has served {chats} chats, using {tokens} tokens and costing ${cost:.4f}. The average latency is {metrics['total_latency_ms']/chats:.2f}ms. In 2 sentences, predict the weekly load and cost if traffic scales to {predicted_weekly_chats} chats per week, and give one performance tip."
            ...
            prediction = f"Based on current trends, scaling to {predicted_weekly_chats} chats/week will cost approximately ${predicted_weekly_cost:.2f} and consume {int(avg_tokens * predicted_weekly_chats)} tokens."
    else:
        prediction = f"Based on current trends, scaling to {predicted_weekly_chats} chats/week will consume {int(avg_tokens * predicted_weekly_chats)} tokens. You are using the local extractive model, so cost is $0.00."
...
async def chat(payload: ChatRequest) -> ChatResponse:
    ...
    metrics["total_estimated_cost_usd"] += response.estimated_cost_usd
    return response
