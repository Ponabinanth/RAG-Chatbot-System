...
class RagChain:
    ...
    def answer(self, question: str, *, top_k: int = 5, history: list[ChatTurn] | None = None) -> ChatResponse:
        ...
        if self.openai_api_key and hits:
            answer, tokens_used = self._openai_answer(question, hits, history or [])
            mode = "openai"
            # GPT-4o-mini pricing approx: $0.15/1M input, $0.60/1M output. Simple blend: $0.0000004 per token.
            cost_usd = tokens_used * 0.0000004 
        else:
            answer, tokens_used = self._extractive_answer(question, hits)
            mode = "extractive"
            cost_usd = 0.0
    ...
    def _extractive_answer(self, question: str, hits: list[dict[str, Any]]) -> tuple[str, int]:
        if not hits:
            return "I could not find relevant context in the indexed documents.", count_tokens(question)
        ...
        answer = "\n".join(lines)
        return answer, count_tokens(question + "\n" + answer)

    def _openai_answer(self, question: str, hits: list[dict[str, Any]], history: list[ChatTurn]) -> tuple[str, int]:
        try:
            ...
            # Use OpenAI returned token count if available, otherwise estimate
            if response.usage:
                tokens = response.usage.total_tokens
            else:
                tokens = input_tokens + count_tokens(answer)
                
            return answer, tokens
        except Exception as exc:
            ans, toks = self._extractive_answer(question, hits)
