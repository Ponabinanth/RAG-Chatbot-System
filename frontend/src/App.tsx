...
import { askQuestion, deleteDocument, fetchDocuments, fetchMetrics, fetchMetricsPrediction, uploadDocuments } from "./api";
import type { ChatMessage, DocumentSummary, Metrics, PredictionResponse } from "./types";
...
function App() {
  ...
  async function handlePredict() {
    setPredicting(true);
    try {
      const res = await fetchMetricsPrediction();
      setPrediction(res.prediction);
    } catch (error) {
      setPrediction(error instanceof Error ? error.message : "Prediction failed");
    } finally {
      setPredicting(false);
    }
  }
          ...
          <aside className="sidebar" aria-label="Document manager">
            ...
            <div className="metric-panel" aria-label="Runtime metrics">
              ...
              <div>
                ...
                <span>{metrics?.total_tokens_used ?? 0} tokens | ${(metrics?.total_estimated_cost_usd ?? 0).toFixed(4)} estimated cost</span>
              </div>
            </div>

            <div className="prediction-panel" style={{ marginTop: '1rem', padding: '1rem', background: 'var(--surface-sunken)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <h4 style={{ margin: 0, fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Sparkles size={16} /> AI Load Prediction</h4>
                <button type="button" onClick={handlePredict} disabled={predicting} style={{ fontSize: '0.8rem', padding: '4px 8px', borderRadius: '4px', background: 'var(--accent)', color: 'white', border: 'none', cursor: 'pointer' }}>
                  {predicting ? 'Predicting...' : 'Predict Load'}
                </button>
              </div>
              {prediction && <p style={{ fontSize: '0.85rem', margin: 0, lineHeight: 1.4, color: 'var(--text-secondary)' }}>{prediction}</p>}
            </div>
