import type { ChatMessage, ChatResponse, DocumentSummary, Metrics, PredictionResponse } from "./types";
...
export async function fetchMetricsPrediction(): Promise<PredictionResponse> {
  const response = await fetch("/api/metrics/predict");
  if (!response.ok) {
    throw new Error("Could not load metrics prediction");
  }
  return response.json();
}
